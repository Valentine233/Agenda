...
TcpSender::TcpSender(QWidget *parent) : QWidget(parent)
{
    loadSize = 4*1024;
    totalBytes = 0;
    bytesWritten = 0;
...
void TcpSender::send()   //连接到服务器，执行发送
{
    ui->sendButton->setEnabled(false);
    bytesWritten = 0;
    //初始化已发送字节为0
...
void TcpSender::on_openButton_clicked() //打开按钮
{
    openFile();
}
void TcpSender::on_sendButton_clicked() //发送按钮
{
    send();
}
